...
3. **Guidance Capabilities** – which full-frame guidance maps (depth, edge, style) the workflow can use.
4. **Variables** – tunable parameters that bind to tokens inside the ComfyUI graph.
5. **Endpoint Requirements** – external models or custom nodes needed at runtime.
6. **Workflow Graph** – the complete ComfyUI node graph that generates imagery.
...
| **`guidance_capabilities`**                 | object          | Declares which full-frame guidance maps are supported (see §4).                                                  |
| **`variables`**                             | array           | Tunable parameters (see §5).                                                                                     |
| **`endpoint_requirements`**                 | object          | External resources required (see §6).                                                                            |
| **`workflow`**                              | object          | The ComfyUI node graph itself (see §7).                                                                          |
...
## 4 • Guidance Capabilities

Workflows can also consume **full-frame guidance maps** supplied by a snapshot (for example: depth maps, edge maps, or a global style image).
Declare which of these inputs your workflow supports.

```json
"guidance_capabilities": {
  "depth": "required" | "optional" | "unsupported",
  "edge":  "required" | "optional" | "unsupported",
  "style": "required" | "optional" | "unsupported"
}
```

* **depth** – grayscale depth map for geometry guidance.
* **edge** – edge or linework map for contour guidance.
* **style** – full-frame style reference image (base64 RGB).

Runners use this section to:

* validate that a snapshot provides the needed guidance,
* drive the UI (for example, enabling depth and edge upload controls only when supported).
...
5. Specify **guidance\_capabilities** so runners know which depth/edge/style maps are expected or optional.
6. Define **variables** with precise `short_description`s and correct `binds_to` tokens.
...
e Pseudotools ecosystem while clearly communicating which guidance maps they support.
